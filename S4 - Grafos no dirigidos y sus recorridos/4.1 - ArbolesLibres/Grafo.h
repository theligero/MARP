...
public:
   ...
   /// <summary>
   /// Devuelve si existe una arista entre el vértice v y
   /// el vértice w.
   /// </summary>
   /// <param name="v">Primer vértice</param>
   /// <param name="w">Segundo vértice</param>
   bool hayArista(int v, int w) const {
       if (v < 0 || v >= _V || w < 0 || v >= _V)
           throw std::domain_error("Vertice(s) inexistente(s)");

       if (_ady[v].size() <= _ady[w].size()) {
           for (auto& u : _ady[v]) {
               if (u == w) return true;
           }
       }
